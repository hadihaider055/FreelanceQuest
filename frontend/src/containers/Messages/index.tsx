...
const MessagesContainer = () => {
    ...
    useEffect(() => {
        if (incomingMessage) {
            if (activeChatMessages && activeChat && incomingMessage.sender_id == activeChat.recipient_member_id) {
                dispatch(setActiveChatMessages([...activeChatMessages, { message: incomingMessage.message, sender_id: incomingMessage.sender_id }]));
            }
    ...
    const connectToPeer = (peerId, userId) => {
        ...
        conn.on("open", () => {
            conn.send(JSON.stringify({userId: session.data?.user.id, connectionOpened: true}))
    ...
    const sendMessage = (userId, message, chat_id) => {
        if (connections[userId]) {
            connections[userId].send(JSON.stringify({ sender_id: session.data.user.id, message: message }));
        }
        ...
        dispatch(setActiveChatMessages([...activeChatMessages, { sender_id: session.data.user.id, message: message }]));
    }
